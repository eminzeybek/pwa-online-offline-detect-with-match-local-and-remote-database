<?php include("inc/duplicateDB.php"); ?>
<!DOCTYPE html>
<html lang="en">
...
<body>
  ...
  <main>
    <h3>Uzak DB'den gelen veriler</h3>
    <ul>
      <?php
      $sqlRemote = $mysqli->query("SELECT * FROM pwa_test ORDER BY id DESC;");
      while($dataRemote = $sqlRemote->fetch_array(MYSQLI_ASSOC)){
      ?>
      <li><?=$dataRemote["name"]?> (<?=$dataRemote["email"]?>)</li>
      <?php } ?>
    </ul>
    ...
    <?php
    $sql = "SELECT * FROM pwa_test WHERE name='Emin' ORDER BY id";
    $kayitlar = $sqlite->query($sql);
    //print_r($kayitlar);
